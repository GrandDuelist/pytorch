    ...
    *,
    ...
    def inner(*flat_args):
        ...
        if gm is None:
            ...
            print(f"Time taken for functionalized execution: {elapsed_time_ms:.2f} ms")
        else:
            ...
            output_nodes = [
                node[0]
                for node in gm.graph.find_nodes(op="output")[0].args
                if len(node) > 0
            ]
            flat_f_outs = tuple(
                node.meta["example_value"]
            ...
            elapsed_time_ms = (time.time() - start_time) * 1000
            print(f"Time taken: {elapsed_time_ms:.2f} ms from graph")
        # print(flat_f_args)
        # print(flat_f_outs)
