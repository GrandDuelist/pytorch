...
def _get_updated_module_call_graph(
    ...
    old_user_inputs = {ix: i for ix, i in enumerate(old_graph_signature.user_inputs)}
    new_user_inputs = {ix: i for ix, i in enumerate(graph_signature.user_inputs)}
    ...
    for node in gm.graph.nodes:
        ...
        # must preserve the old name.
            ...
            if not (
                ...
                if node.target in graph_signature.user_inputs:
                    old_name = old_user_inputs[user_input_counter]
